...
resource sqlsvr 'Microsoft.Sql/servers@2021-02-01-preview' = {
  name: 'adflab-${randomString}-sqlsvr'
  location: location
  properties: {
...
resource sqldb 'Microsoft.Sql/servers/databases@2021-02-01-preview' = {
  parent: sqlsvr
  name: 'adflab-${randomString}-sqldb'
  location: location
  sku: {
    name: 'GP_S_Gen5'
    tier: 'GeneralPurpose'
...
resource kv 'Microsoft.KeyVault/vaults@2021-04-01-preview' = {
  name: 'adflab-${randomString}-keyvault'
  location: location
  properties: {
    sku: {
      family: 'A'
...
resource swsadls 'Microsoft.Storage/storageAccounts@2021-04-01' = {
  name: 'adflab${randomString}synapse'
  location: location
  kind: 'StorageV2'
  sku: {
    name: 'Standard_LRS'
